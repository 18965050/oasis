...
public class AdvancedConsumer {
    ...
    /**
     * 即时返回任务, 如有任务返回的同时将其放入lock job set，如无任务返回null. 如发生redis连接异常, 线程会sleep 5秒后返回null， 如果发生redis数据错误如lua脚本错误，抛出异常.
     */
    ...
    /**
     * 即时批量跑回任务, 如有任务返回的同时将其放入lock job set，如无任务返回空的List. 如发生redis连接异常, 线程会sleep 5秒后返回null， 如果发生redis数据错误如lua脚本错误，抛出异常.
     */
