...
/**
 * Master选举实现, 基于setNx()与expire()两大API. 与每次使用setNx争夺的分布式锁不同，Master用setNX争夺Master
 * Key成功后，会不断的更新key的expireTime，保持自己的master地位，直到自己倒掉了不能再更新为止。 其他Slave会定时检查Master Key是否已过期，如果已过期则重新发起争夺。
 *
