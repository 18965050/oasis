...
public class TripleDesEncryptor implements Encryptor {
    private Logger logger = LoggerFactory.getLogger(TripleDesEncryptor.class);
    ...
    public TripleDesEncryptor() {
        ...
        byteEncryptor.setAlgorithm(Algorithm.PBEWithMD5AndTripleDES.name());
        byteEncryptor.setPassword(DEFAULT_PASSWORD);
        byteEncryptor.initialize();

        stringEncryptor = new StandardPBEStringEncryptor();
        stringEncryptor.setAlgorithm(Algorithm.PBEWithMD5AndTripleDES.name());
