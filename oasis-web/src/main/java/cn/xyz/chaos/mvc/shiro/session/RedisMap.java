...
public class RedisMap<K, V> implements Map<K, V> {
    ...
    // session id(redis hash中一个特殊的field, 对应value存放整个session)
    private final byte[] id;

    public RedisMap(JedisTemplate jedisTemplate, byte[] k, byte[] id) {
    ...
    public Set<Entry<K, V>> entrySet() {
        try {
            return jedisTemplate.execute(new JedisAction<Set<Entry<K, V>>>() {
                @Override
                public Set<Entry<K, V>> action(Jedis jedis) {
    ...
    public boolean isEmpty() {
        ...
        Long size = null;
        try {
            size = jedisTemplate.execute(new JedisAction<Long>() {
                @Override
    ...
    public Collection<V> values() {
        try {
            Collection<byte[]> values = jedisTemplate.execute(new JedisAction<Collection<byte[]>>() {
                @Override
